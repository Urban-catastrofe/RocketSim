...
fn main() {
    let path = std::env::args().nth(1).expect("usage: dump_recording <file.rlpr> [start_tick] [end_tick]");
    let start: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(0);
    let end: usize = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(usize::MAX);
